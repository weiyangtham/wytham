...
date: '2018-02-15'
draft: true
summary: A (shallow) dive into data on NIH scientists
...
pi_opm %>% 
  ...
  labs(y = "Count", x = "Age (5-year bin)", title = "Intramural investigators seem old")
```
...
pi_opm %>% 
  ...
  geom_boxplot(aes(group = yrs_since_degree), alpha = 1/2, outlier.shape = NA, size = 1.1) + 
...
```{r, warning = TRUE}
genni = read_tsv(here::here("../Genni/ssnnamesdata.out"), col_types = cols_only(name = "c", `2008` = "c"))

genni %<>% 
  rename(probs = `2008`) %>% 
  tidyr::separate(probs, into = c("a", "b", "pfem", "d"), sep = "\\|", convert = TRUE) %>% 
  select(name, pfem) %>% 
  mutate(name = str_to_lower(name))

pi_opm %<>% 
  left_join(genni, by = c("first_name" = "name")) 

nafem = pi_opm %>% filter(is.na(pfem)) %>% nrow() # number names without prediction

# pi_opm %>% 
#   ggplot(aes(pfem)) + geom_histogram(na.rm =  FALSE) + 
#   labs(title = "Probability of female based on first name")
...
pi_opm_fem %>% count(female) %>% mutate(percent = n/sum(n) * 100) %>% knitr::kable()
...
mod = lm(adjusted_basic_pay ~ female + factor(yrs_since_degree), data = pi_opm_fem) 
...
gap = lm(adjusted_basic_pay ~ female, data = pi_opm_fem) %>% 
  broom::tidy(conf.int = TRUE) %>% 
  filter(term == "femaleTRUE") %>% 
...
ggplot(gap, aes(x, abs(estimate))) + 
  ...
  coord_flip() + 
  labs(title = "Gender gap remains after accounting for experience")
...
pi_opm_fem %>% 
  ...
  ggplot(aes(x = resid, fill = female)) + 
  geom_density(alpha = 0.5) + 
  # geom_jitter() +
  # geom_boxplot() + 
  # coord_flip() + # geom_boxplot() + 
  labs(title  = "Fatter tails for males")

```

I've plotted the residuals of the simple linear model by gender. The tails of the distribution are fatter for males, indicating that gender pay differences aren't just about mean differences. Of course, a huge caveat in all this is that we're missing a whole lot of useful information (e.g. productivity as measured by publication record) but that's beyond the scope of this blog post.
